...
/* ─────────── 설정 (이 사람에 맞게 바꾸세요) ─────────── */
var FX_FLOAT = ['♡','✿','♡','❀','♡','✿'];   // 떠다니는 입자 모양 (하트+꽃)
var FX_CLICK = '🦋';                          // 클릭하면 터지는 모양 (나비)
  ...
  function build() {
    ...
    if (av && !av.dataset.fxPop) {
      ...
      av.addEventListener('click', function (e) {
        ...
        if (!mqReduce && av.animate) av.animate(
          [{transform:'scale(1)'},{transform:'scale(.88)'},{transform:'scale(1.06)'},{transform:'scale(1)'}],
          { duration:420, easing:'ease-out' });   // 프사 통통 튀기
      });
    }
  }
